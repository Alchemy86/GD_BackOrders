namespace AuctionSniperService.Business.Sites
{
    ...
    public class GoDaddyAuctions : Site
    {

        // FIX TIE DIFF FOR PROGRAM - auction.EndDate > DateTime.Now.AddHours(decimal.ToInt32(2)))
        ...
        private const string SearchString = "https://auctions.godaddy.com/trpSearchResults.aspx";
        ...
        private SortableBindingList<Auctions> LoadMyLocalBids()
        {
            SortableBindingList<Auctions> SavedLocalBids = new SortableBindingList<Auctions>();
        ...
        public IQueryable<AuctionSearch> Search(string searchText)
        {
            ...
            var doc = POST(SearchString, "t=22&action=search&hidAdvSearch=ddlAdvKeyword:1|txtKeyword:" + searchText.Replace(" ", ",") + "&rtr=7&baid=-1&searchMode=1sg=1&showAdult=true&searchDir=1&event=&rnd=0.698455864796415&EqnJYig=561ef36");
            ...
            if (QuerySelectorAll(doc.DocumentNode, "tr.srRow2, tr.srRow1") != null)
            {
                foreach (var node in QuerySelectorAll(doc.DocumentNode, "tr.srRow2, tr.srRow1"))
                {
                    if (QuerySelector(node, "span.OneLinkNoTx") != null && QuerySelector(node, "td:nth-child(5)") != null)
                    {
                        ...
                        auction.DomainName = HTTPDecode(QuerySelector(node, "span.OneLinkNoTx").InnerText);
                        Console.WriteLine(auction.DomainName);

                        auction.BidCount = TryParse_INT(HTTPDecode(QuerySelector(node, "td:nth-child(5)").FirstChild.InnerHtml.Replace("&nbsp;", "")));
        ...
        public string LoginOld(string UserName, string Pwd)
        {
            ...
            _viewstates = string.Format("__VIEWSTATE={0}", HttpUtility.UrlEncode(ExtractViewStateSearch(responseData)));
            //cookies = new CookieContainer();
            request = (HttpWebRequest)WebRequest.Create(strUrl);
            request.Method = "POST";
            request.ContentType = "application/x-www-form-urlencoded";
